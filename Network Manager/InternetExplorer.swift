...
class InternetExplorer{
	...
	func fetchImage(link:String?, callback onComplete: @escaping (Image?) -> Void) {
		...
		// --- Check if image is cache first --
		let cacheKey = NSString(string:link!)
		if let imageIsInCache = self.cache.object(forKey: cacheKey){
			onComplete(Image(uiImage: imageIsInCache))
		}
		...
		self.roamAndFind(link:link){
			...
			//--- save image in cache -----
			self.cache.setObject(newImage!, forKey:cacheKey)
